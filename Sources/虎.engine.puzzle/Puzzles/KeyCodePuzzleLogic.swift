...
class DatePuzzleLogic: GameScene {
	...
	class func newScene(gameLogic: GameLogic) -> DatePuzzleLogic {
        guard let scene = gameLogic.loadScene(scene: "Default.DatePuzzle", resourceBundle: "虎.engine.puzzle", classType: DatePuzzleLogic.classForKeyedUnarchiver()) as? DatePuzzleLogic else {
	...
	func checkPuzleCompleted() {
		...
		if (selectedDate == expectedDate)
		{
            ...
            self.gameLogic?.setScene(sceneIndex: skipToScene, script: nil)
		} else {
